...
public class InfluxQueryBuilderTest {

    private InfluxQueryBuilder influxQueryBuilder;

    @Before
    public void setUp() {
        this.influxQueryBuilder = new InfluxQueryBuilderImpl();
    }
    ...
    public void testSelectWithMultipleOrAndParams() {
        ...
        String query = influxQueryBuilder
                .create()
    ...
    public void testValidParams() {
        String query = influxQueryBuilder.create().select(value()).from("alarms")
        ...
        query = influxQueryBuilder.create().select(value()).from("alarms")
    ...
    public void testSelectCount() {
        String query = influxQueryBuilder.create().select(count()).from("counters")
    ...
    public void testSelectCountValue() {
        String query = influxQueryBuilder.create().select(count("nodeId")).from("counters")
                .where(param("counterId").eqs("4")).build();
    ...
    public void testSubSelect() {
        String query = influxQueryBuilder.create().select(count())
                .from(
                    influxQueryBuilder.create().select(time()).from("alarms"))
    ...
    public void testComplexSubSelect() {
        ...
        String query = influxQueryBuilder.create()
                .select(all())
                .from(
                        influxQueryBuilder.create().select(sum("last"))
                                .from(
                                        influxQueryBuilder.create().select(last(value())).from("counters")
